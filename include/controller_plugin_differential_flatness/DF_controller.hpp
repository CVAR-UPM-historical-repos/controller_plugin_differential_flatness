...
class DFController {
 ...
 public:
  ...
  Vector3d computePositionControl(const UAV_state &state_,
                                  const Control_ref &ref_, const double &dt,
                                  const Vector3d &speed_limit,
                                  const bool &proportional_limitation);

  Vector3d computeVelocityControl(const UAV_state &state_,
                                  const Control_ref &ref_, const double &dt);

  Vector3d computeTrajectoryControl(const UAV_state &state,
