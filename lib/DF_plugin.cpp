...
void Plugin::updateState(const nav_msgs::msg::Odometry &odom) {
  uav_state_.pos =
      Vector3d(odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z);
...
void Plugin::updateReference(const trajectory_msgs::msg::JointTrajectoryPoint &traj_msg) {
  ...
  control_ref_.acc =
      Vector3d(traj_msg.accelerations[0], traj_msg.accelerations[1], traj_msg.accelerations[2]);
...
void Plugin::computeActions(geometry_msgs::msg::PoseStamped &pose,
  ...
  switch (control_mode_in_.control_mode) {
    ...
    case as2_msgs::msg::ControlMode::POSITION:
      // f_des_ = computeSpeed(pose, twist, thrust);
      return;
      break;
