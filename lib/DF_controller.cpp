...
bool DFController::setParametersList(
...
Vector3d DFController::computePositionControl(
    const UAV_state &state, const Control_ref &ref, const double &dt,
    const Vector3d &speed_limit, const bool &proportional_limitation) {
  ...
  if (proportional_limitation) {
    for (short j = 0; j < 3; j++) {
      if (speed_limit[j] == 0.0f || desired_speed[j] == 0.0f) {
        continue;
      };

      if (desired_speed[j] > speed_limit[j] ||
          desired_speed[j] < -speed_limit[j]) {
    ...
    for (short j = 0; j < 3; j++) {
      ...
      desired_speed[j] = (desired_speed[j] < -speed_limit[j])
                             ? -speed_limit[j]
                             : desired_speed[j];
      desired_speed[j] = (desired_speed[j] > speed_limit[j]) ? speed_limit[j]
                                                             : desired_speed[j];
    }
  }
...
Vector3d DFController::computeVelocityControl(const UAV_state &state,
                                              const Control_ref &ref,
                                              const double &dt) {
  // Compute the proportional contribution (velocity)
  Vector3d velocity_error = ref.vel - state.vel;
  Vector3d p_velocity_error_contribution =
      velocity_Kp_lin_mat_ * velocity_error;
  ...
  // Compute the derivative contribution of the error filtered with a first
  // order filter (velocity derivate)
  Vector3d velocity_error_incr = (velocity_error - last_velocity_error_);
  filtered_d_velocity_error_ = alpha_ * velocity_error_incr +
                               (1.0 - alpha_) * filtered_d_velocity_error_;
  Vector3d d_velocity_error_contribution =
      velocity_Kd_lin_mat_ * filtered_d_velocity_error_ / dt;
  ...
  for (short j = 0; j < 3; j++) {
    float antiwindup_value =
        antiwindup_cte_ / velocity_Ki_lin_mat_.diagonal()[j];
    velocity_accum_error_[j] = (velocity_accum_error_[j] > antiwindup_value)
                                   ? antiwindup_value
                                   : velocity_accum_error_[j];
    velocity_accum_error_[j] = (velocity_accum_error_[j] < -antiwindup_value)
                                   ? -antiwindup_value
                                   : velocity_accum_error_[j];
  }
  ...
  Vector3d i_velocity_error_contribution =
      velocity_Ki_lin_mat_ * velocity_accum_error_;

  // Compute desired acceleration
  Vector3d desired_acceleration = p_velocity_error_contribution +
                                  d_velocity_error_contribution +
                                  i_velocity_error_contribution;
...
Vector3d DFController::computeTrajectoryControl(const UAV_state &state,
                                                const Control_ref &ref,
                                                const double &dt) {
  ...
  Vector3d force_error =
      p_error_contribution + d_error_contribution + i_error_contribution;
...
void DFController::computeYawAngleControl(
    // Input
    const UAV_state &state, const float &yaw_angle_ref,
    const Vector3d &force_des,
    // Output
    Vector3d &acro, float &thrust) {
  tf2::Matrix3x3 rot_matrix_tf2(state.rot);

  Eigen::Matrix3d rot_matrix;
  rot_matrix << rot_matrix_tf2[0][0], rot_matrix_tf2[0][1],
      rot_matrix_tf2[0][2], rot_matrix_tf2[1][0], rot_matrix_tf2[1][1],
      rot_matrix_tf2[1][2], rot_matrix_tf2[2][0], rot_matrix_tf2[2][1],
      rot_matrix_tf2[2][2];
...
void DFController::computeYawSpeedControl(
    // Input
    const UAV_state &state, const float &yaw_speed_ref,
    const Vector3d &force_des, const double &dt,
    // Output
    Vector3d &acro, float &thrust) {
  ...
  computeYawAngleControl(state, yaw_angle_ref, force_des, acro, thrust);
...
void DFController::updateGains_() {
  ...
  velocity_Kp_lin_mat_ = Vector3d(parameters_["speed_following.speed_Kp.x"],
                                  parameters_["speed_following.speed_Kp.y"],
                                  parameters_["speed_following.speed_Kp.z"])
                             .asDiagonal();

  velocity_Ki_lin_mat_ = Vector3d(parameters_["speed_following.speed_Ki.x"],
                                  parameters_["speed_following.speed_Ki.y"],
                                  parameters_["speed_following.speed_Ki.z"])
                             .asDiagonal();

  velocity_Kd_lin_mat_ = Vector3d(parameters_["speed_following.speed_Kd.x"],
                                  parameters_["speed_following.speed_Kd.y"],
                                  parameters_["speed_following.speed_Kd.z"])
                             .asDiagonal();

  Kp_ang_mat_ = Vector3d(parameters_["angular_speed_controller.angular_gain.x"],
