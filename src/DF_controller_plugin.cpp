...
void Plugin::updateReference(
  ...
  control_ref_.acc =
      Vector3d(traj_msg.accelerations[0], traj_msg.accelerations[1],
...
void Plugin::computeOutput(geometry_msgs::msg::PoseStamped &pose,
  ...
  if (!flags_.parameters_read) {
    ...
    for (auto &param : parameters_to_read_) {
      RCLCPP_WARN_ONCE(node_ptr_->get_logger(), "Parameter %s not read",
                       param.c_str());
    }
...
void Plugin::computeActions(geometry_msgs::msg::PoseStamped &pose,
  ...
  switch (control_mode_in_.control_mode) {
    case as2_msgs::msg::ControlMode::HOVER:
      f_des_ = controller_handler_->computePositionControl(
          uav_state_, control_ref_, dt, speed_limits_,
    ...
    case as2_msgs::msg::ControlMode::SPEED: {
      if (control_ref_.vel.norm() < 1e-4) {
        ...
        f_des_ = controller_handler_->computePositionControl(
            uav_state_, hover_ref_, dt, speed_limits_,
            proportional_limitation_);
      } else {
        in_hover_ = false;
        f_des_ = controller_handler_->computeVelocityControl(uav_state_,
                                                             control_ref_, dt);
      }
      break;
    }
    case as2_msgs::msg::ControlMode::TRAJECTORY:
      f_des_ = controller_handler_->computeTrajectoryControl(uav_state_,
                                                             control_ref_, dt);
      break;
    default:
